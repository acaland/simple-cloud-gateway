...
function vialactea_tap_sedfit_v7_nospawn,w_in,f_in,df_in,fflag_in,distance,prefilter_thresh,sed_weights=sed_weights,local=local,use_wave=use_wave,outdir=outdir,delta_chi2=delta_chi2

;pro vialactea_tap_sedfit_v7,w_in,f_in,df_in,fflag_in,distance,prefilter_thresh,sed_weights=sed_weights,use_wave=use_wave,outdir=outdir,delta_chi2=delta_chi2
...
curlcmd_start=["curl", "--user", "vialactea:ia2vlkb", "-L", "--data"]
curlrequest='REQUEST=doQuery&VERSION=1.0&LANG=ADQL&FORMAT=csv&QUERY='
...
;  if (fflag(qband) eq 0) then dqlcmd="select id,cluster_id,"+phys_str+","+par_str+" from "+db_schema+"."+db_table+" where ("+col_names(qrefband(0))+" <'"+tostring(f(qband(0))*jy2mjy*fac_resc)+"')"+ul_str ;fluxes are mutliplied by 1000 because model fluxes are in milliJy  
  ...
  curlbody=curlrequest+dqlcmd
  curlcmd=[curlcmd_start,curlbody,curl_endpoint]
  ;print,curlcmd+dqlcmd+curl_endpoint
  print, ""
  print,"Iterazione",bb
  print,curlcmd
  spawn,curlcmd, /NOSHELL, data
  print,"---------------------------------------------------------------------------"
